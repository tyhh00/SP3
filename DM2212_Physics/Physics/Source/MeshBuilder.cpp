...
Mesh* MeshBuilder::GenerateCircle(const std::string& meshName, float radius, Color col)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	...
	for (int theta = 0; theta <= 360; theta += 10)
	{
		...
		v.normal.Set(0, 1, 0);
		v.texCoord.Set(0.5 * cos(Math::DegreeToRadian(theta)) + 0.5f, 0.5 * sin(Math::DegreeToRadian(theta)) + 0.5f);
		vertex_buffer_data.push_back(v);
		v.pos.Set(0, 0, 0);
...
SpriteAnimation* MeshBuilder::GenerateSpriteAnimation(unsigned numRow, unsigned numCol, float tile_width, float tile_height)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
